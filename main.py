...
def mat_encrypt(matrix, rows, cols):
    enc_matrix = np.zeros((int(rows), int(cols)))
    for i in range(rows):
        for ii in range(cols):
            enc_matrix = encrypt(matrix, kappa, p, modulus)
    return enc_matrix
...
def mat_decrypt(matrix,rows,cols):
    ...
    for i in range(rows):
        for ii in range(cols):
            dec_matrix[i][ii] = decrypt(matrix[i][ii], kappa, p)
    return dec_matrix
...
print(A)
# A = mat_decrypt(enc_A,len(A),len(A[0]))
